...
The URL *is* the list state (`code-standards.md` → List UX contract). This phase makes it type-safe.
...
- **One schema, reused by the data layer.** `ticketSearchSchema` is the same Zod object feeding Phase 03's list-query params, so URL and API can't drift:
  ...
  const ticketSearchSchema = z.object({
    ...
    pageSize: z.coerce.number().int().pipe(z.literal([10, 20, 50, 100])).default(20),  // allowlist: URL can't request 100k rows
...
- **Central param updater** in `src/features/tickets/hooks/use-ticket-search-params.ts` — the *only* place list params get written:
  ```ts
  navigate({ search: (prev) => ({ ...prev, ...patch, page: resetsPage(patch) ? 1 : (patch.page ?? prev.page) }) })
  ```
...
- Delete: `src/AppRoutes.tsx`, `src/hooks/use-query-params.ts`, `src/utils/clean-search-params.ts` (superseded by `stripSearchParams`), `use-query-params` + `react-router-dom` + `@types/react-router-dom` + `query-string` deps, `src/features/auth/components/protected-route.tsx` (replaced by guard).
