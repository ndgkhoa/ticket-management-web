...
- **Known trade-off, accepted for this project:** Gemini's free tier states *"Content used to improve our products: Yes"* (paid tier: no). Every ticket here is seeded fake data, so nothing real is exposed. **If this app ever touches real user tickets, this must move to a paid tier or another provider** — record it in the ADR so the decision isn't silently inherited.
...
- Frontend, placed by *what it is*, not by "it's AI":
