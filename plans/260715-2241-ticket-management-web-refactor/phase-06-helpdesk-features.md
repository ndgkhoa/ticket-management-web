...
- **Bulk actions + pagination — decided (industry-standard, per Gmail/GitHub/Zendesk):** selection is **scoped to the current page and clears on page change**. Keyed by `getRowId` (stable ticket id, never row index).
  - The header checkbox selects *this page's* rows only.
  - When the whole page is selected, show the Gmail-style banner: *"All 20 tickets on this page are selected. **Select all N tickets matching these filters**."*
