...
- Reviewed _usage_ of generated database.types.ts (not its content).
...
**P4 structural typing of the builder:** the REAL `supabase.from('tickets').select(TICKET_COLUMNS,{count:'estimated'})` satisfies `SupabaseListQuery<Row>` with **no cast** — `tsc -b` green on a real no-cast call (proved the temp file was in the graph via a deliberate `TS2322`). The `as never` in list-query.test.ts is only because the _fake_ builder is partial; a real Stage-3 ticket api needs no cast.
...
- Code is correct today (nothing imports supabase.ts yet — grep confirms). The risk is the _guidance_: Stage 3's `dataClient` indirection could trust this comment and statically `import { supabase }` on a msw-reachable path, crashing the app at boot in msw/demo/test mode instead of degrading.
